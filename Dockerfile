...
RUN --mount=type=cache,id=s/slack-mcp-server-/go/pkg/mod,target=/go/pkg/mod go mod download

COPY . /app

RUN --mount=type=cache,id=s/slack-mcp-server-/go/pkg/mod,target=/go/pkg/mod \
